...
var anime = (function () {
  ...
  var tweenId = 0;
  function convertKeyframesToTweens(keyframes, target, propertyName, animationType, index, total, groupId, targetPropertyTweens, animationOffsetTime) {
    var prevTween;
    var tweens = [];
    ...
    var _loop = function _loop(i, l) {
      ...
      var previousTargetTween = targetPropertyTweens[sortedIndex - 1];
  ...
  function toggleAnimationDirection(animation) {
    ...
    animation.children.forEach(function (child) {
      return child._isReversed = animation._isReversed;
    });
    return animation;
  }
  ...
  function renderAnimationTweens(animation, time) {
    ...
    while (i < tweensLength) {
      ...
      var tween = tweens[i++];
      if (prevTween && prevTween.groupId == tween.groupId && time < prevTween.end || animation._isRunningBackwards && nextTween && nextTween.groupId == tween.groupId && time > nextTween.start) continue;
      var tweenProgress = tween.easing(clamp(time - tween.start - tween.delay, 0, tween.maxDuration) / tween.duration);
  ...
  function createAnimation(params, parentAnimation) {
    ...
    targets.forEach(function (targetTweens, target) {
      ...
      for (var j = 0, keysLength = propertyKeyframes.length; j < keysLength; j++) {
        ...
        if (is.num(type)) {
          var tweensGroup = convertKeyframesToTweens(keyframes, target, property, type, i, targetsLength, tweensGroupsId, targetPropertyTweens, instanceSettings.timelineOffset);
