...
var anime = (function () {
  ...
  var tweenId = 0;
  function convertKeyframesToTweens(animation, keyframes, target, propertyName, animationType, index) {
    var prevTween;
    var tweens = [];
    var total = animation.targets.size;
    var animationOffsetTime = animation.timelineOffset;
    animation._isOrphan;
    var targetPropertyTweens = animation.targets.get(target)[propertyName];
    ...
    var _loop = function _loop(i, l) {
      ...
      tween.id = tweenId++; // tween.animation = animation;
      ...
      var previousSiblingTween = void 0;
      var previousSiblingTweenAbsoluteEnd = 0;

      if (animation.parent) {
        animation.parent.children.forEach(function (children) {
          var siblingsTarget = children.targets.get(target);

          if (siblingsTarget) {
            var siblingsTweens = siblingsTarget[propertyName];

            if (siblingsTweens) {
              var siblingTweenIndex = 0;

              while (siblingTweenIndex < siblingsTweens.length && siblingsTweens[siblingTweenIndex].absoluteStart - tween.absoluteStart < 0) {
                var curentSibling = siblingsTweens[siblingTweenIndex++];
      ...
      if (previousSiblingTween) {
        if (previousSiblingTween.absoluteEnd >= tween.absoluteStart) {
          ...
          if (previousSiblingTween.endDelay < 0) {
            ...
            if (previousSiblingTween.changeDuration < 0) {
              previousSiblingTween.changeDuration = minValue; // console.log(previousSiblingTween.changeDuration, previousSiblingTween.duration);
            }
        ...
        tween.previous = previousSiblingTween; // if (previousSiblingTween === previousSiblingTween.animation.tweens[previousSiblingTween.animation.tweens.length-1]) {
  ...
  function createAnimation(params, parentAnimation) {
    var parentTargets = parentAnimation ? parentAnimation.targets : new Map();
      ...
      targets.forEach(function (targetTweens, target) {
        ...
        for (var j = 0, keysLength = propertyKeyframes.length; j < keysLength; j++) {
          ...
          if (is.num(type)) {
            ...
            var animationPropertyTweens = convertKeyframesToTweens(animation, keyframes, target, property, type, i);
  ...
  function tickEngine(t) {
    ...
    var activeProcessesLength = engine.children.length;
    var i = 0;
    ...
    while (i < activeProcessesLength) {
      ...
      if (!activeAnimation.paused) {
        ...
        activeProcessesLength--; // console.log(engine.children.length);
      }
    }
  ...
  function createTimeline() {
    ...
    tl.add = function (instanceParams, timelineOffset) {
      var tlIndex = engine.children.indexOf(tl); // TODO: investigate
