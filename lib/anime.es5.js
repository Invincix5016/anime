...
var anime = (function () {
  ...
  function removeAnimationsWithTargets(targetsArray, animations) {
    for (var i = animations.length; i--;) {
      var animationTweens = animations[i].tweens;

      for (var j = animationTweens.length; j--;) {
  ...
  function getAnimations(targets, keyframes, tweenSettings) {
    ...
    var minDelay = !targetsLength ? tweenSettings.delay : null;
      ...
      if (target) {
        ...
        for (var j = 0, keysLength = keyframes.length; j < keysLength; j++) {
          ...
          if (is.num(animationType)) {
            var animationTweens = convertKeyframesToTweens(animationKeyframes, target, tweenPropertyName, animationType, i, targetsLength, tweensGroupsId);
            var firstTween = animationTweens[0];
            var lastTween = animationTweens[animationTweens.length - 1];
            ...
            tweens.push.apply(tweens, _toConsumableArray(animationTweens));
            tweensGroupsId++;
            animationsIndex++;
          }
        }
        ...
        if (!is.und(lastAnimatableTransformAnimationIndex)) {
          animations[lastAnimatableTransformAnimationIndex].tweens.forEach(function (tween) {
  ...
  function createAnimation(params) {
    ...
    var _getAnimations = getAnimations(targets, properties, tweenSettings),
        animations = _getAnimations.animations,
        tweens = _getAnimations.tweens,
        minDelay = _getAnimations.minDelay,
        maxDuration = _getAnimations.maxDuration,
        maxEndDelay = _getAnimations.maxEndDelay;
