...
var anime = (function () {
  ...
  function addNodeToLinkedList(node, list, listType) {
    var lastNode = listType === 'timeline' ? node._tail : node;
    var prevNode = list._tail;
    var nextNode;
  ...
  function tick(t) {
    ...
    while (nextProcess) {
      if (nextProcess._isTween) {
        var animationTime = nextProcess.animation.currentTime;
        var canRender = !(nextProcess._previous && nextProcess._previous.groupId === nextProcess.groupId && animationTime < nextProcess._previous.end);

        if (canRender) {
          var tweenProgress = nextProcess.easing(clamp(animationTime - nextProcess.start - nextProcess.delay, 0, nextProcess.duration) / nextProcess.duration);
          var tweenProperty = nextProcess.property;
          var tweenRound = nextProcess.round;
          var tweenFrom = nextProcess.from;
          var tweenTo = nextProcess.to;
          var tweenType = nextProcess.type;
          var tweenValueType = tweenTo.type;
          var tweenTarget = nextProcess.target;
          var value = void 0;
          ...
          if (tweenValueType == valueTypes.NUMBER) {
            ...
            for (var j = 0, l = tweenTo.numbers.length; j < l; j++) {
              var number = getTweenProgress$1(tweenFrom.numbers[j], tweenTo.numbers[j], tweenProgress, tweenRound);
          ...
          if (tweenType == animationTypes.OBJECT) {
            ...
            if (nextProcess.renderTransforms) {
              ...
              for (var key in nextProcess.cachedTransforms) {
        ...
        var engineTime = (nextProcess.now + (nextProcess.lastTime - nextProcess.startTime)) * settings.speed;
        var insDuration = nextProcess.duration;
        var insChangeStartTime = nextProcess.changeStartTime;
        var insChangeEndTime = insDuration - nextProcess.changeEndTime;
        var animTime = adjustTime(nextProcess, engineTime); // console.log(animTime, engineTime);

        nextProcess.progress = clamp(animTime / insDuration, 0, 1);
        nextProcess.reversePlayback = animTime < animTime; // if (children) { syncInstanceChildren(animTime); }
        ...
        if (animTime > insChangeStartTime && animTime < insChangeEndTime) {
          ...
          nextProcess.change(nextProcess); // setAnimationsProgress(animTime);
        } else {
          if (nextProcess.changeBegan) {
        ...
        if (engineTime >= insDuration) {
          ...
          if (!nextProcess.remainingLoops) {
            ...
            if (!nextProcess.completed) {
              ...
              nextProcess.complete(nextProcess); // nextProcess.resolve();
              // nextProcess.promise = makePromise(nextProcess);
            }
          } else {
            nextProcess.startTime = nextProcess.now;
            nextProcess.loopComplete(nextProcess);
            nextProcess.loopBegan = false; // if (nextProcess.direction === 'alternate') {
  ...
  function createAnimation(params) {
    ...
    var animation = mergeObjects(instanceSettings, {
      id: animationsId++,
      targets: targets,
      children: [],
      tweens: []
      ...
      if (target) {
        ...
        for (var j = 0, keysLength = propertyKeyframes.length; j < keysLength; j++) {
          ...
          if (is.num(type)) {
            ...
            var tweensGroup = convertKeyframesToTweens(keyframes, target, property, type, i, targetsLength, tweensGroupsId);
            ...
            (_animation$tweens = animation.tweens).push.apply(_animation$tweens, _toConsumableArray(tweensGroup));

            tweensGroup.forEach(function (tween) {
  ...
  function animate() {
    ...
    var instance = createAnimation(params);
    ...
    function setInstanceProgress(engineTime) {
      ...
      var insChangeStartTime = instance.changeStartTime;
      var insChangeEndTime = insDuration - instance.changeEndTime;
  ...
  function createTimeline() {
    ...
    tl.add = function (instanceParams, timelineOffset) {
      ...
      children.push(ins); // addNodeToLinkedList(ins, tl, 'timeline');
