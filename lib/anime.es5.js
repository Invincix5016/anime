...
var anime = (function () {
  ...
  function setAnimationProgress(animation) {
    ...
    var engineTime = (animation.now + (animation.lastTime - animation.startTime)) * settings.speed;
    ...
    var animTime = adjustTime(animation, engineTime); // console.log(animTime, engineTime);

    animation.progress = clamp(animTime / insDuration, 0, 1);
    animation.reversePlayback = animTime < animTime; // if (children) { syncInstanceChildren(animTime); }
    ...
    if (animTime > insChangeStartTime && animTime < insChangeEndTime) {
      ...
      animation.change(animation); // setAnimationsProgress(animTime);
    } else {
      if (animation.changeBegan) {
    ...
    if (engineTime >= insDuration) {
      ...
      if (!animation.remainingLoops) {
        ...
        if (!animation.completed) {
          ...
          animation.complete(animation); // animation.resolve();
          // animation.promise = makePromise(animation);
        }
      } else {
        animation.startTime = animation.now;
        animation.loopComplete(animation);
        animation.loopBegan = false; // if (animation.direction === 'alternate') {
  ...
  function setTweenProgress(tween) {
    var animationTime = tween.animation.currentTime;
    var canRender = !(tween._previous && tween._previous.groupId === tween.groupId && animationTime < tween._previous.end);

    if (canRender) {
      var tweenProgress = tween.easing(clamp(animationTime - tween.start - tween.delay, 0, tween.duration) / tween.duration);
  ...
  function tick(t) {
    ...
    var nextProcess = rootTimeline._next;

    while (nextProcess) {
      var cachedNext = nextProcess._next;
