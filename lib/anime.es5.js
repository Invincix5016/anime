...
var anime = (function () {
  ...
  function tick(t) {
    ...
    var activeAnimationsLength = activeAnimations.length;
    var i = 0;

    while (i < activeAnimationsLength) {
      var activeInstance = activeAnimations[i];
  ...
  function handleVisibilityChange() {
    ...
    if (isDocumentHidden()) {
      ...
      activeAnimations.forEach(function (instance) {
  ...
  function registerTargetsToTimeline(targetsParam, timeline) {
    var parsedTargets = parseTargets(targetsParam);
    parsedTargets.forEach(function (target) {
      var storedTarget = timeline.targetsTest.get(target);
  ...
  function removeAnimatablesFromActiveAnimations(targets) {
    ...
    for (var i = activeAnimations.length; i--;) {
  ...
  function getTargetValue(targetSelector, propName, unit) {
    var targets = getTargets(targetSelector);
  ...
  var tweenId = 0;
  function convertKeyframesToTweens(keyframes, target, propertyName, animationType, index, total, groupId, timelineOffset, tweensGroup) {
    var prevTween;
    var tweens = [];
    ...
    var _loop = function _loop(i, l) {
      ...
      for (var j = 0, jl = tweensGroup.length; j < jl; j++) {
        var existingTween = tweensGroup[j];

        if (tween.timelineStart < existingTween.timelineEnd) {
          var overlapTime = existingTween.duration - (existingTween.timelineEnd - tween.timelineStart);
          existingTween.maxProgress = overlapTime / existingTween.duration;
          var newFromValue = getTweenProgress(existingTween.from.number, existingTween.to.number, existingTween.maxProgress);
  ...
  var animationsId = 0;
  var tweensGroupsId = 0;
  function createAnimation(params, timeline) {
    var instanceSettings = replaceObjectProps(defaultAnimationSettings, params);
    var tweenSettings = replaceObjectProps(defaultTweenSettings, params);
    var propertyKeyframes = getKeyframesFromProperties(tweenSettings, params);
    var targets = getTargets(params.targets);
    var targetsLength = targets.length;
    var tweens = [];
    instanceSettings.targetsTest = new Map();
    var registeredTargets = registerTargetsToTimeline(params.targets, timeline ? timeline : instanceSettings);
    var maxDuration = 0;
    var changeStartTime;
    var changeEndTime = 0;
    ...
    for (var i = 0; i < targetsLength; i++) {
      ...
      if (target) {
        var lastTransformGroupIndex = void 0;
        var lastTransformGroupLength = void 0;
        var targetProperties = registeredTargets.get(target);
        ...
        for (var j = 0, keysLength = propertyKeyframes.length; j < keysLength; j++) {
          ...
          var targetPropertyTweens = targetProperties[property];
          ...
          if (is.num(type)) {
            var _targetPropertyTweens;

            var tweensGroup = convertKeyframesToTweens(keyframes, target, property, type, i, targetsLength, tweensGroupsId, instanceSettings.timelineOffset, targetPropertyTweens);
            var tweensGroupLength = tweensGroup.length;
            var firstTween = tweensGroup[0];
            var lastTween = tweensGroup[tweensGroupLength - 1];
            ...
            tweens.push.apply(tweens, _toConsumableArray(tweensGroup));

            (_targetPropertyTweens = targetPropertyTweens).push.apply(_targetPropertyTweens, _toConsumableArray(tweensGroup));
  ...
  function animate() {
    ...
    var timeline = arguments.length > 1 ? arguments[1] : undefined;
    var startTime = 0,
        lastTime = 0,
        now = 0;
    var children,
    ...
    var animation = createAnimation(params, timeline);
    makePromise(animation);
    ...
    function toggleAnimationDirection() {
      ...
      children.forEach(function (child) {
        return child.reversed = animation.reversed;
      });
    }
    ...
    function setTweensProgress(insTime) {
      ...
      while (i < tweensLength) {
        var prevTween = tweens[i - 1];
        var nextTween = tweens[i + 1];
        var tween = tweens[i++];
        if (prevTween && prevTween.groupId == tween.groupId && insTime < prevTween.end || animation.reversePlayback && (nextTween && nextTween.groupId == tween.groupId && insTime > nextTween.start || insTime > tween.end)) continue;
    ...
    function setAnimationProgress(engineTime, manual) {
      var insDuration = animation.duration;
      var insChangeStartTime = animation.changeStartTime;
      var insChangeEndTime = insDuration - animation.changeEndTime;
      var insTime = adjustTime(engineTime);
      var renderTime = insTime;
      var canRender = 0;
      animation.progress = clamp(insTime / insDuration, 0, 1);
      animation.reversePlayback = insTime < animation.currentTime;

      if (children) {
        syncAnimationChildren(insTime, false, manual);
      }
    ...
    animation.seek = function (time, muteCallbacks) {
      if (muteCallbacks) {
        if (children) {
          syncAnimationChildren(time, true);
        }
  ...
  function getTimingsFromAnimationsOrInstances(animationsOrInstances, tweenSettings) {
    ...
    if (!animationsLength) {
      ...
      for (var i = 0; i < animationsLength; i++) {
        ...
        var changeStartTime = timelineOffset + anim.changeStartTime;
        ...
        var changeEndTime = timelineOffset + anim.duration - anim.changeEndTime;
  ...
  function createTimeline() {
    ...
    tl.add = function (instanceParams, timelineOffset) {
      var tlIndex = activeAnimations.indexOf(tl);
