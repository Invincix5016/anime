...
var anime = (function () {
  ...
  function tick(t) {
    ...
    while (i < activeAnimationsLength) {
      var activeAnimation = activeAnimations[i];
  ...
  function animate() {
    ...
    var children,
        childrenLength = 0; // let resolve = null;
    ...
    var animation = createAnimation(params); // let promise = makePromise(animation);
    ...
    function syncAnimationChildren(time, ignoreCallbacks) {
      if (!animation.reversePlayback) {
        for (var i = 0; i < childrenLength; i++) {
          seekChild(time, children[i], ignoreCallbacks);
        }
      } else {
        for (var j = childrenLength; j--;) {
          seekChild(time, children[j], ignoreCallbacks);
        }
      }
    }
    ...
    function setAnimationProgress(engineTime, ignoreCallbacks) {
      ...
      var needsRender = ignoreCallbacks;
      animation.progress = clamp(insTime / insDuration, 0, 1);

      if (children) {
        syncAnimationChildren(insTime, ignoreCallbacks);
      }
      ...
      if (engineTime >= insDuration) {
        ...
        if (!animation.remainingLoops) {
          ...
          if (!animation.completed) {
            ...
            animation.complete(animation); // resolve();
    ...
    animation.reset = function () {
      ...
      if (animation.reversed && animation.loop !== true || direction === 'alternate' && animation.loop === 1) animation.remainingLoops++; // setTweensProgress(animation.reversed ? animation.duration : 0);
