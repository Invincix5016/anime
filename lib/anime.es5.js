...
var anime = (function () {
  ...
  function syncAnimationChildren(animation, time, muteCallbacks) {
    if (!(time < animation.currentTime)) {
      ...
      for (var j = animation._childrenLength; j--;) {
        ...
        _child.seek(time - _child.timelineOffset, muteCallbacks, true);
      }
    }
  }
  function renderAnimationTweens(animation, time, isSeekingBackwards) {
    ...
    while (i < animation._tweensLength) {
      ...
      animation._frameInterval < 33.33333 && tween.isOverlapped && absTime > tween.absoluteChangeEnd || tween.previous && absTime < tween.previous.absoluteChangeEnd || tween.next && absTime > tween.next.absoluteStart) continue;
  ...
  function tickEngine(t) {
    ...
    var activeAnimationsLength = engine.children.length;
    var i = 0;
  ...
  function removeAnimatablesFromActiveAnimations(targets) {
    ...
    for (var i = engine.children.length; i--;) {
      var animation = engine.children[i];
      removeTweensWithTargetsFromAnimation(targetsSet, animation);
    }
  }
  ...
  function seek(animation, time, muteCallbacks, isSeekingBackwards) {
    if (muteCallbacks) {
