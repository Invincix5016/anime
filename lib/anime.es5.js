...
var anime = (function () {
  ...
  function removeTweensWithTargets(targetsSet, animation) {
    for (var i = animation.tweens.length; i--;) {
  ...
  function convertPropertyValueToKeyframes(property, propertyValue, duration, delay, endDelay, easing, round) {
  ...
  var tweenId = 0;
  function convertKeyframesToTweens(animation, keyframes, target, tweenSiblings, propertyName, animationType, index) {
    var prevTween;
    var tweens = [];
    var total = animation.targets.size;
    var timelineOffset = animation.timelineOffset;
    animation._isOrphan;
    ...
    var _loop = function _loop(i, l) {
      ...
      var ti = 0;
      var parentPreviousSiblingTween = void 0;
      var parentPreviousSiblingTweenAbsoluteEnd = 0;

      while (ti < tweenSiblings.length && (!tweenSiblings[ti].isSkipped && tweenSiblings[ti].absoluteStart - tween.absoluteStart) < 0) {
        var curentSibling = tweenSiblings[ti++];
        var curentSiblingAbsoluteEnd = curentSibling.absoluteEnd;
      ...
      var previousSiblingTween = prevTween || parentPreviousSiblingTween;
      ...
      if (previousSiblingTween) {
        ...
        if (previousSiblingTween.absoluteEnd >= tween.absoluteStart) {
          ...
          var next = previousSiblingTween.next;
  ...
  function syncAnimationChildren(animation, time, muteCallbacks) {
    var isRunningBackwards = time < animation.currentTime;

    for (var i = 0; i < animation._childrenLength; i++) {
      var child = animation.children[!isRunningBackwards ? i : animation._childrenLength - 1 - i];
      seekAnimation(child, time - child.timelineOffset, muteCallbacks);
    }
  }
  function renderAnimationTweens(animation, time, isSeekingBackwards) {
    var i = 0;
    var tweens = animation.tweens;
    var absTime = animation.timelineOffset + time;

    while (i < animation._tweensLength) {
      var tween = tweens[i++]; // tween.isUpdating = false; // For GUI debug only

      if (tween.isOverridden || // TODO : Cleanup animation._frameInterval < 33.33333 and find a better way to test is a jump happened
      // (animation._frameInterval < 33.33333 && tween.isOverlapped && absTime > tween.absoluteChangeEnd) ||
      tween.previous && absTime < tween.previous.absoluteChangeEnd || tween.next && absTime > tween.next.absoluteStart) continue; // tween.isUpdating = true; // For GUI debug only

      var tweenProgress = tween.easing(clamp(time - tween._changeStartTime, 0, tween.changeDuration) / tween.updateDuration);
  ...
  function setAnimationProgress(animation, parentTime, isSeekingBackwards) {
    var animationDuration = animation.duration;
    var animationChangeStartTime = animation._changeStartTime;
    var animationChangeEndTime = animation._changeEndTime;
    ...
    if (animation._childrenLength) {
      syncAnimationChildren(animation, animationTime, 0);
    }
    ...
    animation.currentTime = clamp(animationTime, 0, animationDuration); // animation._frameInterval = Math.abs(animation._lastFrameTime -  animation.currentTime);
    // animation.isUpdating = canRender; // For GUI debug only

    if (canRender) renderAnimationTweens(animation, animation.currentTime); // animation._lastFrameTime = animation.currentTime;
  ...
  function seekAnimation(animation, time, muteCallbacks, isSeekingBackwards) {
    if (muteCallbacks) {
  ...
  function tickAnimation(animation, t) {
  ...
  function createAnimation(parameters, parentAnimation) {
    // const parentTargets = parentAnimation ? parentAnimation.targets : new Map();
    var params = parameters.keyframes ? mergeObjects(flattenParamsKeyframes(parameters.keyframes), parameters) : parameters;
    var update = params.update,
        begin = params.begin,
        loopBegin = params.loopBegin,
        changeBegin = params.changeBegin,
        change = params.change,
        changeComplete = params.changeComplete,
        loopComplete = params.loopComplete,
        ...
        round = params.round;
    var parent = parentAnimation || engine;
    var targets = registerTargetsToMap(params.targets, parent.targets);
    var tDuration = !is.und(duration) ? duration : defaultTweenSettings.duration;
    var tDelay = !is.und(delay) ? delay : defaultTweenSettings.delay;
    var tEndDelay = !is.und(endDelay) ? endDelay : defaultTweenSettings.endDelay;
    var tEasing = easing || defaultTweenSettings.easing;
    var tRound = !is.und(round) ? round : defaultTweenSettings.round;
    var animation = {
      params: parameters,
      update: update || noop,
      begin: begin || noop,
      loopBegin: loopBegin || noop,
      changeBegin: changeBegin || noop,
      change: change || noop,
      changeComplete: changeComplete || noop,
      loopComplete: loopComplete || noop,
      complete: complete || noop,
      loop: !is.und(loop) ? loop : defaultAnimationSettings.loop,
      direction: direction || defaultAnimationSettings.direction,
      autoplay: !is.und(autoplay) ? autoplay : defaultAnimationSettings.autoplay,
      timelineOffset: !parentAnimation ? performance.now() : !is.und(timelineOffset) ? timelineOffset : 0,
      id: animationsId++,
      targets: targets,
      parent: parent,
      tweens: [],
      children: [],
      duration: tDelay + tDuration + tEndDelay,
      ...
      _lastFrameTime: 0,
      // Used to calculate _frameInterval to check if the animation is playing continuously or if there was a jump in time
      _frameInterval: 0,
      // Used to check if the animation is playing continuously or if there was a jump in time
      _changeStartTime: tDelay,
      _changeEndTime: tDelay + tDuration,
      _childrenLength: 0
    };
    ...
    if (targets.size) {
      ...
      targets.forEach(function (targetTweens, target) {
        ...
        for (var p in params) {
          if (is.key(p)) {
            var keyframes = convertPropertyValueToKeyframes(p, params[p], tDuration, tDelay, tEndDelay, tEasing, tRound);
            ...
            if (is.num(type)) {
              var _animation$tweens;

              var animationPropertyTweens = convertKeyframesToTweens(animation, keyframes, target, targetPropertyTweens, property, type, i);
              ...
              var lastTweenCET = lastTween._changeEndTime;
              ...
              (_animation$tweens = animation.tweens).push.apply(_animation$tweens, _toConsumableArray(animationPropertyTweens));
            }
          }
        }
  ...
  engine.tick = function (t) {
    ...
    while (i < engine.children.length) {
      var activeAnimation = engine.children[i];
      ...
      if (!activeAnimation.paused) {
        ...
        for (var j = activeAnimation.tweens.length; j--;) {
          var tween = activeAnimation.tweens[j];
          var parentTargetTweens = activeAnimation.parent.targets.get(tween.target)[tween.property];

          for (var k = parentTargetTweens.length; k--;) {
  ...
  function animate() {
    ...
    animation.seek = function (time, muteCallbacks, isSeekingBackwards) {
      return seekAnimation(animation, time, muteCallbacks);
    };
    ...
    animation.tick = function (t) {
      return tickAnimation(animation, t);
    };
  ...
  function createTimeline() {
    var tlParams = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : {};
    var tl = animate(tlParams);
    ...
    tl.add = function (animationParams, timelineOffset) {
      ...
      var animParams = mergeObjects(animationParams, replaceObjectProps(defaultTweenSettings, tlParams));
      animParams.targets = animParams.targets || tlParams.targets;
      animParams.autoplay = false; // animParams.direction = tl.direction; // TODO: investigate
      ...
      for (var i = 0, l = tl._childrenLength; i < l; i++) {
        var child = tl.children[i];
        var childTLOffset = child.timelineOffset;
        var childCST = childTLOffset + child._changeStartTime;
        ...
        var childCET = childTLOffset + child._changeEndTime;
  ...
  anime.remove = function (targets) {
    ...
    for (var i = engine.children.length; i--;) {
      removeTweensWithTargetsFromAnimation(targetsSet, engine.children[i]);
    }
  };
