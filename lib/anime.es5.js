...
var anime = (function () {
  ...
  function handleVisibilityChange() {
    ...
    if (isDocumentHidden()) {
      ...
      engine.activeProcesses.forEach(function (instance) {
  ...
  function animate() {
    ...
    function renderAnimationTweens(tweens, time) {
      ...
      while (i < tweensLength) {
        ...
        var tween = tweens[i++];
        if (prevTween && prevTween.groupId == tween.groupId && time < prevTween.end || animation.reversePlayback && nextTween && nextTween.groupId == tween.groupId && time > nextTween.start) continue;
        var tweenProgress = tween.easing(clamp(time - tween.start - tween.delay, 0, tween.duration) / tween.duration);
    ...
    function setAnimationProgress(animation, parentTime, manual) {
      ...
      var animationChangeStartTime = animation.changeStartTime;
      var animationChangeEndTime = animationDuration - animation.changeEndTime;
      var animationTime = getAdjustedAnimationTime(animation, parentTime);
      var renderTime = animationTime;
      var canRender = 0;
      ...
      if (animation.children) {
        syncAnimationChildren(animation, animationTime, false, manual);
      }
    ...
    animation.reset = function () {
      ...
      animation.finished = window.Promise && new Promise(function (resolve) {
        return animation.resolve = resolve;
      });

      for (var i = animation.childrenLength; i--;) {
    ...
    animation._onDocumentVisibility = function () {
    ...
    animation.seek = function (time, muteCallbacks) {
      if (muteCallbacks) {
        if (animation.children) {
          syncAnimationChildren(animation, time, true);
        }
