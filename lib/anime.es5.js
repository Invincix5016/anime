...
var anime = (function () {
  ...
  function createAnimation(params) {
    ...
    var keyframes = getKeyframesFromProperties(tweenSettings, params);
    var targets = getAnimatables(params.targets);
    var targetsLength = targets.length;
    var minDelay = !targetsLength ? tweenSettings.delay : undefined;
    var maxDuration = !targetsLength ? tweenSettings.duration : 0;
    var maxEndDelay = !targetsLength ? tweenSettings.delay : 0;
    var tweens = [];
    ...
    for (var i = 0; i < targetsLength; i++) {
      ...
      if (target) {
        var lastTransformFirstTweenIndex = void 0;
        var lastTransformLastTweenIndex = void 0;

        for (var j = 0, keysLength = keyframes.length; j < keysLength; j++) {
          var tweenKeyframes = keyframes[j];
          var tweenKeyframesPropertyName = tweenKeyframes[0].property;
          var tweenType = getAnimationType(target, tweenKeyframesPropertyName);
          var tweenPropertyName = sanitizePropertyName(tweenKeyframesPropertyName, target, tweenType);

          if (is.num(tweenType)) {
            var tweensGroup = convertKeyframesToTweens(tweenKeyframes, target, tweenPropertyName, tweenType, i, targetsLength, tweensGroupsId);
            var tweensGroupLength = tweensGroup.length;
            var firstTween = tweensGroup[0];
            var lastTween = tweensGroup[tweensGroupLength - 1];
            var endDelay = lastTween.end - lastTween.endDelay;
        ...
        if (!is.und(lastTransformFirstTweenIndex)) {
          for (var t = lastTransformFirstTweenIndex; t < lastTransformLastTweenIndex; t++) {
