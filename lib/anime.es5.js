...
var anime = (function () {
  ...
  function convertKeyframesToTweens(keyframes, target, propertyName, animationType, index, total) {
    var prevTween;
    var tweens = [];
      ...
      if (target) {
        ...
        for (var j = 0, keysLength = keyframes.length; j < keysLength; j++) {
          ...
          if (is.num(animationType)) {
            var animationTweens = convertKeyframesToTweens(animationKeyframes, target, tweenPropertyName, animationType, i, targetsLength);
            ...
            animationTweens.forEach(function (tween) {
  ...
  function createInstance(params) {
    ...
    var _getAnimations = getAnimations(targets, properties),
  ...
  function animate() {
    ...
    function setAnimationsProgress(insTime) {
      var i = 0; // const animations = instance.animations;
      ...
      while (i < tweensLength) {
        ...
        var tween = tweens[i++];
        tweens[i];
        if (prevTween && prevTween.groupId == tween.groupId && insTime < prevTween.end) continue; // if (nextTween && (nextTween.groupId === nextTween.groupId)) {
        ...
        if (tweenType == animationTypes.OBJECT) {
          ...
          var cached = cache.DOM.get(tweenTarget);
